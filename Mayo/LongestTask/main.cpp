			...
			for(int i=0; i<idleWorkers.size(); i++){
				...
				if(idleWorkers[i].getTech()){
					...
					if(idleWorkers[i].getIV()){
						...
						string lTask="NONE";
						if(entryQ.size()>0){
							if(entryQ.front().getEntryTime() > longest){
								longest=entryQ.front().getEntryTime();
								lTask="Entry";
							}
						}
						if(oralFillQ.size()>0){
							if(oralFillQ.front().getFillTime()>longest){
								longest=oralFillQ.front().getFillTime();
								lTask="Fill";
							}
						}
						if(dispQ.size()>0){
							if(dispQ.front().getDispTime() > longest){
								longest=dispQ.front().getDispTime();
								lTask="Dispense";
							}
						}
						if(longest==0){
					...
					string lTask="NONE";
					if(entryVerQ.size()>0){
						if(entryVerQ.front().getEntryVerTime() > longest){
							longest=entryVerQ.front().getEntryVerTime();
							lTask="EntryVer";
						}
					}
					if(fillVerQ.size()>0){
						if(fillVerQ.front().getFillVerTime() > longest){
							longest=fillVerQ.front().getFillVerTime();
							lTask="FillVer";
						}
					}
					if(entryQ.size()>0){
