...
int main(){
	...
	for(int simCount=0; simCount<numSim; simCount++){
		...
		double val;
		ifstream oralIn("./inputs/oralIn.txt");
		while (oralIn >> val ){
			oralIncoming.push(val);
		}
		oralIn.close();
		ifstream ivIn("./inputs/ivIn.txt");
		while (ivIn >> val ){
			ivIncoming.push(val);
		}
		ivIn.close();
		ifstream entryIn("./inputs/entry.txt");
		while (entryIn >> val ){
			entryTimes.push(val);
		}
		entryIn.close();
		ifstream entryVerIn("./inputs/entryver.txt");
		while (entryVerIn >> val ){
			entryVerTimes.push(val);
		}
		entryVerIn.close();
		ifstream oralP("./inputs/oralprep.txt");
		while (oralP >> val ){
			oralPrepTimes.push(val);
		}
		oralP.close();
		ifstream ivP("./inputs/ivprep.txt");
		while (ivP >> val ){
			ivPrepTimes.push(val);
		}
		ivP.close();
		ifstream pVer("./inputs/prepver.txt");
		while (pVer >> val ){
			prepVerTimes.push(val);
		}
		pVer.close();
		ifstream d("./inputs/disp.txt");
		while (d >> val ){
			dispenseTimes.push(val);
		}
		d.close();
